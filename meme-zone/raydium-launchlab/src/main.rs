...
/// Raydium Launch Lab program ID - the main program responsible for token creation and trading
const RAYDIUM_LAUNCH_LAB_PROGRAM_ID: &str = "LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj";
...
impl TokenTracker {
    ...
    pub fn record_transaction(&self, mint: String, signature: String, sol_amount: f64, timestamp: DateTime<Utc>, transaction_type: RaydiumInstructionType) {
        debug!("Recording {} transaction for token {}: {:.4} SOL", 
               format!("{:?}", transaction_type), mint, sol_amount);
        
        // Log the transaction details
        match transaction_type {
            RaydiumInstructionType::Buy | RaydiumInstructionType::Create => {
            ...
            RaydiumInstructionType::MigrateAMM | RaydiumInstructionType::MigrateCPSwap => {
                info!("🚀 Migration Transaction: {}", mint);
                info!("  Signature: {}", signature);
                info!("  Timestamp: {}", timestamp);
            }
            RaydiumInstructionType::Sell => {
...
struct ProcessedTransaction {
    ...
    raydium_instruction_type: Option<RaydiumInstructionType>,
    /// Processing timestamp
    processed_at: DateTime<Utc>,
}
...
impl BondingCurveState {
    ...
    pub fn calculate_market_cap_quote(&self) -> f64 {
    ...
    /// Calculate quote token needed to reach graduation threshold
    pub fn quote_needed_for_graduation(&self) -> f64 {
        let current_quote = self.real_quote as f64 / (10u64.pow(self.quote_decimals as u32) as f64);
        (GRADUATION_QUOTE_THRESHOLD - current_quote).max(0.0)
    }
...
impl GraduationTracker {
    ...
    pub fn update_bonding_curve(&self, mint: String, state: BondingCurveState) {
        ...
        let _market_cap = state.calculate_market_cap_quote();
        let _sol_needed = state.quote_needed_for_graduation();
    ...
    fn log_graduation_alert(&self, mint: &str, state: &BondingCurveState, threshold: f64) {
        ...
        let market_cap = state.calculate_market_cap_quote();
        let sol_needed = state.quote_needed_for_graduation();
        let current_sol = state.real_quote as f64 / 1_000_000_000.0;
        
        warn!("🎯 GRADUATION ALERT ({}%): {}", threshold, mint);
        info!("  Graduation progress: {:.2}%", progress);
        info!("  Current market cap: {:.4} SOL", market_cap);
        info!("  SOL collected: {:.4} / {:.1}", current_sol, GRADUATION_QUOTE_THRESHOLD);
        info!("  SOL needed: {:.4}", sol_needed);
        info!("  Virtual token reserves: {}", state.virtual_base);
        info!("  Real token reserves: {}", state.real_base);
    ...
    pub fn get_tokens_close_to_graduation(&self) -> Vec<(String, f64, f64)> {
        self.bonding_curves.iter()
            .filter_map(|entry| {
                ...
                let progress = state.calculate_graduation_progress();
                if progress >= 50.0 {
                    Some((mint.clone(), progress, state.calculate_market_cap_quote()))
                } else {
                    None
                }
            })
    ...
    pub fn get_graduation_stats(&self) -> (usize, usize, usize) {
        ...
        let completed = self.bonding_curves.iter()
            .filter(|entry| entry.value().status == 1)
            .count();
...
impl SolanaBlockScanner {
    ...
    async fn process_block_transactions(
        ...
        for (tx_index, transaction) in transactions.iter().enumerate() {
            ...
            if self.is_raydium_transaction(transaction) {
                ...
                if self.has_instruction_type(transaction, &CREATE_INSTRUCTION_DISCRIMINATOR) ||
                   ...
                   self.has_instruction_type(transaction, &MIGRATE_AMM_INSTRUCTION_DISCRIMINATOR) ||
                   self.has_instruction_type(transaction, &MIGRATE_CPSWAP_INSTRUCTION_DISCRIMINATOR) {
                    ...
                    if !is_successful {
                        debug!("Skipping failed Raydium Launch Lab transaction");
                        continue;
                    }
                    
                    // Process and log successful Raydium Launch Lab transactions
                    match self.process_single_transaction(transaction, processed_block, tx_index).await {
                        Ok(processed_tx) => {
                            // Handle CREATE and BUY transactions
                            if processed_tx.raydium_instruction_type == Some(RaydiumInstructionType::Create) {
                                let tx_type = processed_tx.raydium_instruction_type.as_ref().unwrap();
                            ...
                            } else if processed_tx.raydium_instruction_type == Some(RaydiumInstructionType::Buy) ||
                                processed_tx.raydium_instruction_type == Some(RaydiumInstructionType::Sell) {
                                    let tx_type = processed_tx.raydium_instruction_type.as_ref().unwrap();
                            ...
                            } else if processed_tx.raydium_instruction_type == Some(RaydiumInstructionType::MigrateAMM) ||
                                processed_tx.raydium_instruction_type == Some(RaydiumInstructionType::MigrateCPSwap) {
                                info!("Found Pump.fun MIGRATE transaction: {}", processed_tx.signature);

                                // Extract mint address for tracking
                                if let Some(mint) = self.extract_mint_address_from_transaction(transaction, &RaydiumInstructionType::MigrateAMM) {
                                    info!("  Token Mint: {}", mint);
                                    ...
                                    self.token_tracker.record_transaction(
                                        ...
                                        processed_tx.processed_at,
                                        RaydiumInstructionType::MigrateAMM
                                    );
                                }

                                if let Some(mint) = self.extract_mint_address_from_transaction(transaction, &RaydiumInstructionType::MigrateCPSwap) {
                                    info!("  Token Mint: {}", mint);
                                    ...
                                    self.token_tracker.record_transaction(
                                        ...
                                        0.0, // No SOL spent in migrations
                                        processed_tx.processed_at,
                                        RaydiumInstructionType::MigrateCPSwap
                                    );
                                }
    ...
    /// Check if a transaction involves the Raydium Launch Lab program
    ///
    /// # Arguments
                ...
                match &ui_transaction.message {
                    solana_transaction_status::UiMessage::Parsed(parsed_message) => {
                        ...
                        parsed_message.account_keys.iter()
                            .any(|key| key.pubkey == RAYDIUM_LAUNCH_LAB_PROGRAM_ID)
                    }
                    solana_transaction_status::UiMessage::Raw(raw_message) => {
                        // Check if Pump.fun program ID is in account keys
                        raw_message.account_keys.iter()
                            .any(|key| key == RAYDIUM_LAUNCH_LAB_PROGRAM_ID)
                    }
                }
            }
            _ => {
                        ...
                        for instruction in &parsed_message.instructions {
                            ...
                            let (_program_id, data) = match instruction {
                                ...
                                solana_transaction_status::UiInstruction::Compiled(compiled) => {
                                    ...
                                    if program_id != RAYDIUM_LAUNCH_LAB_PROGRAM_ID {
                                        continue;
                                    }
    ...
    async fn process_single_transaction(
        ...
        let is_pump_fun_transaction = account_keys.iter().any(|key| key == RAYDIUM_LAUNCH_LAB_PROGRAM_ID);
        let pump_fun_instruction_type = if is_pump_fun_transaction {
            self.determine_raydium_instruction_type(transaction)
        } else {
            None
        };
        ...
        Ok(ProcessedTransaction {
            ...
            raydium_instruction_type: pump_fun_instruction_type,
    ...
    fn extract_mint_address_from_transaction(
        &self, 
        transaction: &solana_transaction_status::EncodedTransactionWithStatusMeta,
        instruction_type: &RaydiumInstructionType
    ) -> Option<String> {
        // Get the appropriate discriminator for the instruction type
        let discriminator = match instruction_type {
            RaydiumInstructionType::Buy => &BUY_INSTRUCTION_DISCRIMINATOR,
            RaydiumInstructionType::Sell => &SELL_INSTRUCTION_DISCRIMINATOR,
            RaydiumInstructionType::Create => &CREATE_INSTRUCTION_DISCRIMINATOR,
            RaydiumInstructionType::MigrateAMM => &MIGRATE_AMM_INSTRUCTION_DISCRIMINATOR,
            RaydiumInstructionType::MigrateCPSwap => &MIGRATE_CPSWAP_INSTRUCTION_DISCRIMINATOR,
            _ => return None,
        };
                ...
                match BondingCurveState::deserialize(&mut data_slice) {
                    Ok(mut bonding_curve_state) => {
                        ...
                        debug!("Successfully extracted bonding curve state for {}: real_sol_reserves={} lamports ({:.4} SOL), real_token_reserves={}, complete={}", 
                              ...
                              bonding_curve_state.real_base,
                              bonding_curve_state.real_base as f64 / 1_000_000_000.0,
                              bonding_curve_state.real_quote,
                              bonding_curve_state.status == 1,
                              );
